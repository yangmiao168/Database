 ...
 */
//                                           level/numKeys      sibling ptr             key               rid
const  int INTARRAYLEAFSIZE = ( Page::SIZE -2*sizeof( int )- sizeof( PageId ) ) / ( sizeof( int ) + sizeof( RecordId ) );

/**
 * @brief Number of key slots in B+Tree non-leaf for INTEGER key.
 */
//                                               level/numKeys     extra pageNo                  key       pageNo
...
private:
	...
	/**
	 * Inserts the new entry (Key and RecordId or Key and PageNo) to the targeted node
	 * which is determined by level.
	 *
	 * @param targetNode       		the target Node that the entry will inserted into
	 * @param newKey        		Key for insert
	 * @param newId					RecordId or PageNo for insert
	 * @param nodeLevel			    Level of the node.
	 */

	const void insertNonFullNode(void *targetNode, const void *newKey, void *newId, int nodeLevel);

	/**
	 * Recursively inserts the entries into the B+ tree, if the node is full, split the node.
	 *
	 * @param key        		Key for insert.
	 * @param rid        		Record ID of the entry.
	 * @param middleKey			middle key of the node, split position
	 * @param currPageNo		pageNo which is used to recursively check for data entry.
	 * @return PageId		    if splitting occurred, return the page number of new child node, otherwise return 0.
	 */

	PageId insert (const void *key, const RecordId rid, void *middleKey, PageId currPageNo);
